...
export default class DownloadTask {
  ...
  _onBegin ({ expectedBytes, headers }) {
    ...
    if (this._beginHandler) this._beginHandler({ expectedBytes, headers })
  }
  ...
  _onDone ({ location }) {
    this.state = 'DONE'
    if (this._doneHandler) this._doneHandler({ location })
  }

  _onError (error, errorCode) {
    this.state = 'FAILED'
    if (this._errorHandler) this._errorHandler(error, errorCode)
  }
